/* Problem: Alice has n candies, where the ith candy is of type candyType[i]. Alice noticed that she started to gain weight, so she visited a doctor.
  ...
  while still following the doctor's advice. Given the integer array candyType of length n,
...
function distributeCandies(candyType) {
	...
	for (var i = 0; i < n; i++) {
		hashMap[candyType[i]] = true;
	}

	var noOfUniqueCandies = Object.keys(hashMap).length;

	return noOfUniqueCandies >= n / 2 ? n / 2 : noOfUniqueCandies;
}

distributeCandies([1, 1, 2, 2, 3, 3]); // 3
distributeCandies([1, 1, 2, 2, 3, 3]); // 2
distributeCandies([6, 6, 6, 6]); // 1

// Time Complexity: O(N)
// Space Complexity: O(N), worst case all the candies in candyType is unique
