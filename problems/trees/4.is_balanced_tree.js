...
var isBalanced = function(root) {
	var getHeight = function(root, height) {
		...
		if (leftTree < 0 || rightTree < 0 || Math.abs(leftTree - rightTree) > 1) {
			return -1; // Math.abs to get positive number, incase if its in -ve number
		}
	...
	// Initial height as 0
	return getHeight(root, 0) >= 0; // If retured value is >= 0 then it is binary tree with height balanced.
};
